...
auto
nanos_between(time_point const a, time_point const b)
{
...
Ty
get_required_arg(char const *const argname, bpo::variables_map const &args)
{
  if (args.count(argname) == 0) {
    std::cerr << "missing required argument --" << argname;
    EXIT(exit_code::MISSING_REQUIRED_ARGUMENT);
  } else {
...
template <typename Ty>
std::optional<Ty>
get_optional_arg(char const *const argname, bpo::variables_map const &args)
{
  if (args.count(argname) == 0) {
    return std::nullopt;
  } else {
...
b8
get_flag_arg(char const *const argname, bpo::variables_map const &args)
{
  ...
  simulation::configuration const &cfg)
{
  ...
  while (true) {
    ...
    u64 const gens_completed = cfg.generation;
    ...
    b8 const is_simulation_done = !cfg.can_step_forward(generation_target);
    ...
    if (ticks == 0 || is_simulation_done) {
      ...
      if (gens_remaining > 0) {
        ...
        printf(fore::DEFAULT | back::BLACK, "finished, %s", [&cfg]() {
          switch (cfg.last_step_res) {
...
main(int const argc, char const *const *const argv)
{
  ...
  std::string const sim_file_pathname = get_required_arg<std::string>("scenario", args);
  u64 const generation_target = get_required_arg<u64>("gentarget", args);
  ...
  if (img_fmt == pgm8::format::NIL) {
    ...
    EXIT(exit_code::BAD_ARGUMENT_VALUE);
  }

  simulation::configuration cfg;
  strvec_t errors{};

  cfg = simulation::parse_config(
  ...
  std::thread ui_update_thread(update_ui, sim_name, generation_target, std::ref(cfg));
